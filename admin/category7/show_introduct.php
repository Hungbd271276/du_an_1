      ...
      try{
        ...
        <tr><th>ID</th><th>Tên </th><th>Ảnh</th><th>Chi tiết</th> <th>Update</th> <th>Delete</th></tr>";
         ...
         foreach($mang as $row){
            ...
            $link_update = "?page=introduct&action=update&id=".$row['id'];
            $link_del = "?page=introduct&action=del&id=".$row['id']; 
            echo "<tr> <td>{$row['id']}</td> <td>{$row['tieu_de']}</td>  <td><img src = '$link_avt' width='150px'/></> <td>{$row['content']}</td>
