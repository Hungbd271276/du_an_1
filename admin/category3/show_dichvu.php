...
try{
  ...
  foreach($mang as $row){
    ...
    $link_update = '?page=dich-vu&action=update&id='.$row['ma_dv'];
    $link_del = '?page=dich-vu&action=del&id='.$row['ma_dv'];
    echo "<tr> <td>{$row['ma_dv']}</td> <td>{$row['ten_dv']}</td> <td>{$row['gia']}</td> <td><img src = '$link_avt' width='100px'/></td> <td>{$row['chi_tiet']}</td> 
